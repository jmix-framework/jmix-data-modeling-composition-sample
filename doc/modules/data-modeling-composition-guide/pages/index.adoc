...
=== Association: Loosely Coupled Entities
...
=== Composition: Master-Detail Relationships
...
----
<1> The `editorBuffered="true"` attribute ensures that editing changes are not saved immediately but are buffered until explicitly saved by the user. This provides control over the editing process. `allRowsVisible="true"` allows the `DataGrid` to dynamically expand its height as new rows are added.

<2> Columns have to be marked as `editable`. In our case all columns `editable="true"` as we also want to create new addresses directly in the data grid.

<3> An `editorActionsColumn` is added to handle row-specific editing. It includes options to enable editing, save changes, or cancel edits. This keeps the editing process intuitive and visually clear for users.
...
=== Transactional Behavior and the Role of the DataContext

The transactional behavior in Jmix is made possible by the `DataContext`. The `DataContext` acts as a temporary storage for entities that are being created, modified, or deleted within a view. Instead of persisting changes directly to the database, the `DataContext` holds these changes in memory until the user saves the root entity of the view.

In the `Owner` detail view, any changes to the associated `Address` entities are managed by the `DataContext`. For example, when a new address is added or an existing one is edited, these changes are stored temporarily. Only when the user confirms their changes by clicking "OK" on the overall view, all the modifications saved to the database in a single transaction. If the user cancels the view, the `DataContext` discards all pending changes, ensuring that no unintended modifications are persisted.

This mechanism allows for greater flexibility and a user-friendly editing experience. Users can safely experiment with their changes, knowing that nothing is committed until they explicitly confirm. At the same time, this ensures data consistency by persisting all changes in a single transaction.

You can read more about the `DataContext` in the User Interface section: xref:flow-ui:data/data-context.adoc[Data Components: DataContext].


[NOTE]
====
Using inline editing for compositions helps maintain a clear and intuitive user experience. However, for complex nested structures, dialog-based editing might be more suitable. We will explore such scenarios in later sections.
====
