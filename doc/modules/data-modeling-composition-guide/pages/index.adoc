...
To maintain the parent-child relationship in the Data Context, dialog-based navigation is used. This approach ensures that child views inherit the Data Context from their parent, preserving transactional consistency.
...
<3> The `withParentDataContext` ensures that the new `HealthRecord` shares the same `dataContext` as the `PetDetailView`. This means the new record remains in a transient state until explicitly saved, ensuring that the health record is only saved once the Pet is saved.
